# Creating Colored Elements #

Each element is a table that you can customize, with a default color of **255 255 255 (255 Alpha)**. You can create colorful elements like this:

```lua
local element = NativeUI.CreateColouredItem('Element Name', 'Element Description',{R = 240, G = 240, B = 240, A = 255},{R = 0, G = 0, B = 0, A = 255})
```

The first value after the description represents the normal color of the element, while the second value indicates the color when it's hovered over. In our example, the element would be black when not hovered over and white when hovered over! 🎨

But wait, there's more! You can also apply colors to different texts in NativeUI using the GTA5 color code style. For example, ~g~ would make the text green, and the possibilities are diverse...

If you're feeling adventurous, you can even manually edit the color of an element, but that's a more advanced technique. Don't worry! This exciting process is clearly illustrated and explained in the element section of our wiki. 🌈

[Source](https://github.com/MrMathias154/NativeUILua-Reloaded/blob/master/elements/Colours.lua)
